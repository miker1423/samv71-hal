...
type SerialRegisterBlock = atsamv71q21::uart0::RegisterBlock;
...
fn read(uart: *const SerialRegisterBlock) -> nb::Result<u8, UartError> {
    ...
    } else if status_register.rxrdy().bit_is_set() {
    ...
    ($($UART:ident: ($uart:ident, $uarttx: ident, $uartrx:ident),)+) => {
        $(
            ...
            impl<TXPIN, RXPIN> ConfigMethod for Serial<$UART, TXPIN, RXPIN> {
                type Parity = atsamv71q21::$uart::mr::PAR_A;
                type Mode = atsamv71q21::$uart::mr::CHMODE_A;
...
uart! { UART0: (uart0, uart0tx, uart0rx), }
uart! { UART1: (uart1, uart1tx, uart1rx), }
uart! { UART2: (uart2, uart2tx, uart2rx), }
uart! { UART3: (uart3, uart3tx, uart3rx), }
uart! { UART4: (uart4, uart4tx, uart4rx), }
